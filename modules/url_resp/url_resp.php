...
function getContent($data)
{
	...
	$last_entry_count = 10;
	$script_path = realpath(__DIR__) . '/scripts/start.php';
	...
	$con .= '
		...
		$con .= '<div class="http_link_to_script"><b>' . $locale->getLocale('http_link_to_script', $lng) . "</b>: <span style='color:#0000ff'>http://" . $_SERVER['SERVER_NAME'] . dirname($_SERVER['PHP_SELF']) . '/modules/' . $module . '/scripts/start.php</span></div>';  
		...
		$con .= '<div class="last_entry"><b>' .  $locale->getLocale('last_entry_all', $lng) . ' (' . $last_entry_count . '):</b></div>';
						
		$log_data = getLog('all', $last_entry_count, $script_path);	
		...
		for($l = 0; $l < $count_log_data; $l++)
		{
			...
			if($l == 0)
				$con .= '<div><span class="data_class">' . getSelectDatetime($log_data[$l]['datetime']) . '</span>:' . $log_file . ' <b>[' . $log_data[$l]['chat_id'] . ']</b> ' . $log_data[$l]['notice'] . '</div>';
			else
				$con .= '<div><span class="data_class">' . $log_data[$l]['datetime'] . '</span>: ' . $log_file . ' <b>[' . $log_data[$l]['chat_id'] . ']</b> ' . $log_data[$l]['notice'] . '</div>';
		...
		if($admin_chat_id != "")
		{
			$con .= '<div class="last_entry"><b>' .  $locale->getLocale('last_entry', $lng) . ' [' . $admin_chat_id . '] (' . $last_entry_count . '):</b></div>';
